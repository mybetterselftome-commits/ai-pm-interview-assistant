...
MASTERY_EXAMPLES = [
    ("RAG", "RAG 适合需要查企业知识库、合同资料或项目文档的场景。产品上要先检索资料，再让模型基于资料回答，重点关注切分、召回、引用来源和人工复核。"),
    ("Agent", "Agent 是围绕目标连续完成任务的 AI 系统，不只是聊天。产品设计要关注任务流程、工具调用、权限边界、失败兜底和最终任务完成率。"),
    ("模型评估", "模型评估要结合产品场景定义标准，不只是看回答通不通顺。常见指标包括准确率、采纳率、人工修改率、错误率、响应速度和用户满意度。"),
    ("Prompt 工程", "Prompt 工程不是写一句提示词，而是通过角色、任务、输入边界、输出格式和示例约束模型，让输出更稳定、更可评估。"),
    ("Function Calling", "Function Calling 适合让模型调用外部工具完成查询、创建、计算等动作。产品上要设计参数校验、权限控制、失败重试和人工确认。"),
    ("模型幻觉", "模型幻觉是指模型生成看似合理但不真实的内容。产品上可以通过 RAG、引用来源、置信度提示、人工审核和用户反馈降低风险。"),
    ("向量数据库", "向量数据库用于存储文本向量，支持语义检索。产品经理不用深究算法，但要理解它服务于相似内容召回和知识库问答。"),
    ("Embedding", "Embedding 是把文本转成向量表示，方便计算语义相似度。它常用于知识检索、推荐、去重和相似问题匹配。"),
    ("Chunking", "Chunking 是把长文档切成适合检索的小段。切得太粗会召回不准，切得太细会丢上下文，所以要结合业务文档结构设计。"),
    ("召回率", "召回率表示系统是否把相关资料找出来。RAG 产品中召回率低会导致模型基于错误或缺失资料回答。"),
    ("准确率", "准确率关注 AI 输出是否正确。不同场景准确率定义不同，比如合同审查看风险识别是否正确，客服看回答是否解决问题。"),
    ("采纳率", "采纳率表示用户是否接受 AI 的建议或结果。它比生成次数更能说明 AI 功能是否真的有用。"),
    ("人工复核", "人工复核适合高风险场景。产品设计上要明确哪些结果必须人工确认，哪些可以自动通过。"),
    ("置信度", "置信度用于提示 AI 对结果的把握程度，但不能盲信。产品上要结合来源、规则和人工反馈一起判断。"),
    ("灰度发布", "灰度发布是先让小范围用户使用新功能，观察效果和风险，再逐步扩大。AI 功能尤其适合灰度，因为输出不稳定性更高。"),
    ("A/B 测试", "A/B 测试用于比较不同方案的效果，比如不同提示词、不同推荐策略或不同交互方式对转化率的影响。"),
    ("人机协同", "人机协同不是让 AI 全自动，而是让 AI 处理重复、初筛、生成或提示，人负责判断、确认和高风险决策。"),
    ("人工兜底", "人工兜底是 AI 失败时的安全出口，比如转人工客服、提示用户补充信息、提供手动编辑入口。"),
    ("任务完成率", "任务完成率衡量用户是否真的完成目标，比点击率更重要。Agent 产品尤其需要看任务是否闭环完成。"),
    ("转人工率", "转人工率常用于客服或 Agent 场景。过高说明 AI 解决能力不足，过低也可能说明风险被隐藏。"),
    ("用户反馈闭环", "用户反馈闭环是把用户的赞踩、修改、投诉和补充信息转成下一轮优化材料，帮助产品和模型持续改进。"),
    ("知识库问答", "知识库问答不是简单聊天，而是基于企业资料回答问题。产品要关注资料导入、权限、召回、引用和答案可信度。"),
    ("智能客服", "智能客服的核心不是替代人工，而是提升常见问题处理效率，并在复杂或高风险问题上顺畅转人工。"),
    ("AI 助手", "AI 助手要围绕具体任务设计，而不是做万能聊天。产品经理要定义用户目标、输入输出、边界和成功指标。"),
    ("AI 搜索", "AI 搜索强调理解用户意图并组织答案。它和传统搜索不同，除了召回内容，还要生成可理解的总结。"),
    ("AI 内容生成", "AI 内容生成要关注质量、风格、合规和人工审核。不能只看生成速度，还要看用户是否采纳和修改多少。"),
    ("内容审核", "内容审核场景要平衡效率和风险。AI 可做初筛，但违规边界、误杀率、漏检率和人工复审都要设计清楚。"),
    ("模型选型", "模型选型要看效果、成本、速度、稳定性、隐私和接入难度，而不是只选最强模型。"),
    ("成本控制", "AI 产品成本包括模型调用、向量检索、存储和人工复核。产品经理要用缓存、分级模型和场景分流控制成本。"),
    ("响应延迟", "响应延迟会影响用户体验。可以用流式输出、异步任务、预生成、缓存和简化输出降低体感等待。"),
    ("多轮对话", "多轮对话需要维护上下文，但也容易跑偏。产品上要设计上下文范围、澄清问题和重置机制。"),
    ("记忆机制", "记忆机制用于保留用户偏好或历史信息，但要控制隐私、可编辑和可删除，不能无限记住所有内容。"),
    ("权限控制", "企业 AI 产品必须考虑权限控制，用户只能检索和生成自己有权访问的资料。"),
    ("数据安全", "数据安全包括敏感信息保护、访问权限、日志审计和模型调用边界，尤其适合企业场景。"),
    ("日志埋点", "日志埋点用于记录用户关键行为，比如上传、生成、采纳、导出、反馈，帮助判断产品是否真正被使用。"),
    ("效果看板", "效果看板把产品指标、模型指标和业务指标放在一起，帮助团队判断 AI 功能是否值得继续迭代。"),
    ("需求澄清", "AI 需求常常很模糊，产品经理要先问清用户、场景、输入、输出、成功标准和不能接受的风险。"),
    ("MVP", "AI 产品 MVP 应该先验证最关键的用户任务，不要一开始做大而全。重点是证明 AI 是否能解决真实问题。"),
    ("PRD", "AI 产品 PRD 除了普通需求，还要写清 AI 能力边界、输入输出、评估方式、兜底策略和人工流程。"),
    ("用户旅程", "用户旅程帮助识别 AI 应该介入哪个环节。不是所有步骤都适合自动化，有些更适合辅助决策。"),
    ("业务指标", "业务指标反映产品是否创造价值，比如转化率、留存、处理时长、节省成本、满意度。"),
    ("模型指标", "模型指标关注 AI 本身表现，比如准确率、召回率、拒答率、错误率、响应时间。"),
    ("离线评测", "离线评测是在上线前用测试集评估模型效果，适合比较不同提示词、模型和检索策略。"),
    ("线上评估", "线上评估看真实用户使用效果，比如采纳率、投诉率、完成率和反馈质量。"),
    ("提示词模板", "提示词模板可以让不同用户稳定获得相似结构的输出，适合需要标准化报告或分析的产品。"),
    ("低代码平台", "低代码平台适合快速验证 AI 产品原型，但面试中要说明它的边界，比如可扩展性、权限和数据安全。"),
    ("Dify", "Dify 是常见 AI 应用搭建平台，可用于快速做知识库问答或工作流原型，但不能替代产品方案设计。"),
    ("Coze", "Coze 可用于搭建 Bot 和简单 Agent 原型，适合展示流程想法，但要补充产品指标和业务场景。"),
    ("合同审查", "合同审查类 AI 产品适合用 RAG 和结构化抽取，但必须设计引用原文、风险等级、人工确认和责任边界。"),
    ("成本分析", "成本分析类 AI 产品可以辅助整理表格、识别偏差和生成风险提示，但关键结果要允许业务人员复核。"),
]
