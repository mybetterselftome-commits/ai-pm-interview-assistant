...
MASTERY_EXAMPLES = [
    ("RAG", "RAG 适合需要查企业知识库、合同资料或项目文档的场景。产品上要先检索相关资料，再让模型基于资料回答，重点关注资料切分、召回质量、引用来源和人工复核。"),
    ("Agent", "Agent 是能围绕一个目标连续完成任务的 AI 系统，不只是聊天。产品设计要关注任务流程、工具调用、权限边界、失败兜底和任务完成率。"),
    ("模型评估", "模型评估不是只看回答好不好，而是要结合产品场景定义标准。常见指标包括准确率、采纳率、人工修改率、错误率、响应速度和用户满意度。"),
    ("Prompt 工程", "Prompt 工程不是写一句提示词，而是通过角色、任务、输入边界、输出格式和示例约束模型，让输出更稳定、更可评估。"),
    ("Function Calling", "Function Calling 适合让模型调用外部工具完成查询、计算、创建工单等动作。产品上要设计参数校验、权限控制、失败重试和人工确认。"),
    ("模型幻觉", "模型幻觉是模型生成看似合理但不真实的内容。产品上可以通过 RAG、引用来源、置信度提示、人工审核和用户反馈来降低风险。"),
    ...
    ("召回率", "召回率表示系统是否把相关资料找出来。RAG 产品中召回率低会导致模型基于错误或缺失资料回答，是知识库效果评估的关键指标。"),
    ("采纳率", "采纳率表示用户是否接受 AI 的建议或结果。它比生成次数更能说明 AI 功能是否真的有用，适合评估写作、分析和推荐类产品。"),
    ("人工复核", "人工复核适合高风险场景。产品设计上要明确哪些结果必须人工确认，哪些可以自动通过，以及复核结果如何反哺系统。"),
    ("人机协同", "人机协同不是让 AI 全自动，而是让 AI 处理重复、初筛、生成或提示，人负责判断、确认和高风险决策。"),
    ("人工兜底", "人工兜底是 AI 失败时的安全出口，比如转人工客服、提示用户补充信息、提供手动编辑入口或回到传统流程。"),
    ("任务完成率", "任务完成率衡量用户是否真的完成目标，比点击率更重要。Agent、客服、资料分析类产品都应该关注任务是否闭环完成。"),
    ...
    ("模型选型", "模型选型要看效果、成本、速度、稳定性、隐私和接入难度，而不是只选最强模型。不同任务可以用不同模型分层处理。"),
    ("成本控制", "AI 产品成本包括模型调用、向量检索、存储和人工复核。产品经理要用缓存、分级模型、场景分流和输出长度控制成本。"),
    ("响应延迟", "响应延迟会影响用户体验。可以用流式输出、异步任务、预生成、缓存和简化输出降低体感等待。"),
    ("多轮对话", "多轮对话需要维护上下文，但也容易跑偏。产品上要设计上下文范围、澄清问题、重置机制和用户确认节点。"),
    ("权限控制", "企业 AI 产品必须考虑权限控制，用户只能检索和生成自己有权访问的资料，避免越权查看敏感信息。"),
    ("数据安全", "数据安全包括敏感信息保护、访问权限、日志审计和模型调用边界，尤其适合企业知识库、合同和成本分析场景。"),
